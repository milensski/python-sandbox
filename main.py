...
CONTAINER_NAME = "sandbox-" + str(hash(USER_CODE))

client = docker.from_env()

with open("data/full_code.py", "w+") as code_file:
    code_file.write(FULL_CODE)

container = client.containers.run(BASE_IMAGE, ["python", "/data/full_code.py"],
                                  volumes={"E:\\exercise\\data": {'bind': '/data', 'mode': 'rw'}}, detach=True)
time.sleep(1)

print(container.logs().decode("utf-8").strip('.').strip('-'))
